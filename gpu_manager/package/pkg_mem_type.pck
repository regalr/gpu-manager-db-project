create or replace package pkg_mem_type is
  ...
  procedure update_mem_type(p_mem_id   number,
...
CREATE OR REPLACE PACKAGE BODY pkg_mem_type IS
  ...
  begin
    ...
    select nvl(p_mem_name, mem_name)
      into new_mem_name
      from mem_type s
     where s.mem_id = p_mem_id;
  
    update mem_type set mem_name = new_mem_name where mem_id = p_mem_id;
  ...
  begin
    ...
    if p_mem_id is null then
      ...
      SELECT ty_mem_type(t.mem_id, t.mem_name)
        BULK COLLECT
        INTO v_list
        from mem_type t;
