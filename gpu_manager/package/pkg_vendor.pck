create or replace package pkg_vendor is
  ...
  procedure add_vendor(p_vendor_name varchar2, p_country varchar2);

  procedure update_vendor(p_vendor_id   number,
...
CREATE OR REPLACE PACKAGE BODY pkg_vendor IS
  ...
  --add
  procedure add_vendor(p_vendor_name varchar2, p_country varchar2) is
  begin
    insert into vendor
      (vendor_id, vendor_name, country)
    
    values
      (null, p_vendor_name, p_country);
  ...
  begin
    ...
    select nvl(p_vendor_name, vendor_name)
      into new_vendor_name
      from vendor v
    ...
    update vendor
       set vendor_name = new_vendor_name, country = new_country
  ...
  begin
    ...
    if p_vendor_id is null then
      ...
      SELECT ty_vendor(t.vendor_id, t.vendor_name, t.country)
        BULK COLLECT
        INTO v_list
        from vendor t;
