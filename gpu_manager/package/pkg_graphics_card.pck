create or replace package pkg_graphics_card is
  ...
  function show_graphics_card(p_graphics_card_id number default null)
    return ty_graphics_card_l;
...
CREATE OR REPLACE PACKAGE BODY pkg_graphics_card IS
  ...
  begin
    ...
    is_compatible := pkg_functions.memory_compatibility(p_mem_size,
                                                        p_mem_int_width);
  ...
  exception
    when pkg_error.ex_incompatible_mem then
      RAISE_APPLICATION_ERROR(pkg_error.ex_incompatible_mem_code,
                              'Helytelen memoria konfiguracio!');
  ...
  begin
    ...
    select nvl(p_graphics_card_name, graphics_card_name)
      into new_graphics_card_name
      from graphics_card s
     where s.graphics_card_id = p_graphics_card_id;
    select nvl(p_gpu_chip, gpu_chip)
      into new_gpu_chip
      from graphics_card s
     where s.graphics_card_id = p_graphics_card_id;
    select nvl(p_first_release, first_release)
      into new_first_release
      from graphics_card s
     where s.graphics_card_id = p_graphics_card_id;
    select nvl(p_mem_size, mem_size)
      into new_mem_size
      from graphics_card s
     where s.graphics_card_id = p_graphics_card_id;
    select nvl(p_mem_id, mem_id)
      into new_mem_id
      from graphics_card s
     where s.graphics_card_id = p_graphics_card_id;
    select nvl(p_mem_int_width, mem_int_width)
      into new_mem_int_width
      from graphics_card s
     where s.graphics_card_id = p_graphics_card_id;
    select nvl(p_mem_clock, mem_clock)
      into new_mem_clock
      from graphics_card s
     where s.graphics_card_id = p_graphics_card_id;
    select nvl(p_base_clock, base_clock)
      into new_base_clock
      from graphics_card s
     where s.graphics_card_id = p_graphics_card_id;
    select nvl(p_boost_clock, boost_clock)
      into new_boost_clock
      from graphics_card s
     where s.graphics_card_id = p_graphics_card_id;
    select nvl(p_manufacturing_cost, manufacturing_cost)
      into new_manufacturing_cost
      from graphics_card s
     where s.graphics_card_id = p_graphics_card_id;
  
    is_compatible := pkg_functions.memory_compatibility(new_mem_size,
                                                        new_mem_int_width);
    ...
    update graphics_card
       set graphics_card_name = new_graphics_card_name,
