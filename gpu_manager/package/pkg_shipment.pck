create or replace package pkg_shipment is
  ...
  procedure update_shipment(p_shipment_id        number,
  ...
  function show_shipment(p_shipment_id number default null)
    return ty_shipment_l;
...
CREATE OR REPLACE PACKAGE BODY pkg_shipment IS
  ...
  procedure add_shipment(p_vendor_id        number,
    ...
    cost number;
  begin
    select distinct g.manufacturing_cost
      into cost
      from shipment s
  ...
  begin
    ...
    select nvl(p_vendor_id, vendor_id)
      into new_vendor_id
      from shipment s
     where s.shipment_id = p_shipment_id;
  
    select nvl(p_graphics_card_id, graphics_card_id)
      into new_graphics_card_id
      from shipment s
     where s.shipment_id = p_shipment_id;
  
    select nvl(p_quantity, quantity)
      into new_quantity
      from shipment s
     where s.shipment_id = p_shipment_id;
  
    select nvl(p_shipment_date, shipment_date)
      into new_shipment_date
      from shipment s
     where s.shipment_id = p_shipment_id;
  
    select nvl(p_price_per_unit, price_per_unit)
      into new_price_per_unit
      from shipment s
     where s.shipment_id = p_shipment_id;
